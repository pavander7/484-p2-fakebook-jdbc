...
-- Query 5a
-- Goals: (A)
CREATE OR REPLACE PROCEDURE query_5a (num_results IN NUMBER, year_diff IN NUMBER) AS
BEGIN
    SELECT  u1.user_id, 
            u1.first_name1, 
            u1.last_name, 
    ...
    -- ensures users are different & that pairs are only selected once
    ...
    -- (iii) difference in birth years is no more than <yearDiff>
    AND ABS(u1.year_of_birth - u2.year_of_birth) <= year_diff -- FIXME: replace with a variable in java
    -- (iv) not friends
    AND NOT EXISTS (
        SELECT 1
        ...
        -- u1.user_id < u2.user_id & f.user1_id < f.user2_id -> 
        -- no need to check if f.user2_id = u1.user_id & f.user1_id = u2.user_id
    )
...
-- Query 5b
-- Goals: (B)
CREATE OR REPLACE PROCEDURE query_5b (target_user1 IN NUMBER, target_user2 IN NUMBER) AS
BEGIN
    SELECT  p.photo_id, 
            p.photo_link, 
            a.album_id, 
    ...
    WHERE EXISTS(
        ...
        WHERE t.tag_subject_id = target_user1 -- FIXME: replace with a variable in java
        AND t.tag_photo_id = p.photo_id
    ) 
    AND EXISTS(
        SELECT 1
        FROM project2.Public_Tags t -- PublicFakebookConstants.TagsTable t
        WHERE t.tag_subject_id = target_user2 -- FIXME: replace with a variable in java
        AND t.tag_photo_id = p.photo_id
    ) 
    ORDER BY p.photo_id;
END query_5b;
